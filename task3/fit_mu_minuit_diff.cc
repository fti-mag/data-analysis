...
Double_t func(Double_t *x, Double_t *par) {
   Double_t _par[8];
   _par[0] = 0.5*(par[0]+par[1]);
   _par[1] = par[2];
   _par[2] = par[3];
   _par[3] = par[4];
   _par[4] = 0.5*(par[0]-par[1]);
   _par[5] = par[5];
   _par[6] = par[6];
   _par[7] = par[7];
   return func_mu(x, _par) + func_mu(x, _par + 4);
}
...
void fit_mu_minuit_diff() {
  ...
  const Double_t vstart[npar] = {6666.0,0.0,120.0,80.0,0.2,35.0,5.0,-0.2};
  const Double_t step[npar] = {10,10,1,0.1,0.01,1,0.1,0.01};
  const char *name[npar] = {"Ne+Mmu", "Ne-Nmu", "Ee", "Se", "Ae", "Emu", "Smu", "Amu"};
